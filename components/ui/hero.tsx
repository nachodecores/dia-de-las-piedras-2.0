...
export function Hero() {
  ...
  return (
    <section
      className="relative min-h-[55vh] min-h-[520px] w-full overflow-hidden pb-2 md:min-h-[480px] mb-8 md:mb-10 bg-cover bg-center"
      style={{ backgroundImage: "url(/fondoconfeti.png)" }}
    >
      {SLIDES.map((slide, index) => (
        <div
          key={index}
          className="absolute inset-0 flex flex-col items-center justify-center px-6 py-12 text-center transition-opacity duration-700 ease-in-out"
          style={{
        ...
        {SLIDES.map((_, index) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-300 ${
              index === current ? "w-6 bg-[#1F2A44]" : "w-2 bg-[#1F2A44]/50 hover:bg-[#1F2A44]/70"
            }`}
          />
        ))}
      </div>
